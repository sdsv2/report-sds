...
exports.nominal = function (nominal, rsNumber, topupQty, callback) {
  ...
  return new Promise(function(resolve, reject) {
    let sql = "SELECT * FROM topup INNER JOIN stock_ref USING (stock_ref_id) INNER JOIN rs_chip USING (rs_id) INNER JOIN sd_chip USING (sd_id) WHERE nominal=? AND rs_number=? AND topup_qty=? AND topup_status in ('P','S') AND log_id IS NULL AND DATE(topup_ts)=DATE(NOW())";
...
exports.rsChipByRsOutletId = function (rsOutletId, callback) {
  // body...
  return new Promise(function(resolve, reject) {
    let sql = "SELECT * FROM rs_chip WHERE rs_outlet_id=?";
    connection.query(sql, [rsOutletId], function(err, rows, fields){
      if(err){
        common.log("query rs chip by rs outlet id "+err.message);
        throw err.message;
        return;
      }else{
        if(!rows[0]){
          callback('failed', 'nothing rs chip for '+rsOutletId);
          resolve();
        }else{
          callback('', rows);
          resolve();
